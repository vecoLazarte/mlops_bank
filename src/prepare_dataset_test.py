...
class PrepareData():
    ...
    def __init__(self, output_path, preprocess_train_run_id):
    ...
    def prepare_impute_missing(self, df_data, x_cols):
        ...
        imputacion_parametros_path = download_artifacts(run_id=self._preprocess_train_run_id, artifact_path=f"{self._output_path}/imputacion_parametros.csv")
    ...
    def apply_label_encoders_to_test(self, df_test):
        ...
        path_encoder = download_artifacts(run_id=self._preprocess_train_run_id, artifact_path=f"{self._output_path}/label_encoder_train.pkl")
    ...
    def aplicar_estandarizacion_test(self, df_test):
        path_scaler = download_artifacts(run_id=self._preprocess_train_run_id, artifact_path=f"{self._output_path}/scaler_train.pkl")
...
def process_prepare_dataset(tracking_uri, experiment_name, parent_run_id, split_run_id, preprocess_train_run_id):
    ...
    with mlflow.start_run(run_id=parent_run_id):
        with mlflow.start_run(run_name="prepare_dataset_test", nested=True) as prepare_dataset_test:
            prepare_dataset_test_id = prepare_dataset_test.info.run_id
            clientes_test_path = download_artifacts(run_id=split_run_id, artifact_path="data/out/clientes_data_test.csv")
            df_data_test = pd.read_csv(clientes_test_path)
            requerimiento_test_path = download_artifacts(run_id=split_run_id, artifact_path="data/out/requerimientos_data_test.csv")
            df_requerimientos_test = pd.read_csv(requerimiento_test_path)
            prepare_data_instance = PrepareData("outputs/preprocess",preprocess_train_run_id)
            ...
            mlflow.log_artifact('data/out/data_test_prepared.csv', artifact_path=f'data/out')
    return prepare_dataset_test_id
